...
char **load_strings_mmap(const char *filename, size_t count,
  ...
  /* Параметры для mmap:
   * 1) Выбор подходящего адреса для отображения
   * 2) Размер самого файла, сколько нужно отобразить байтов
   * 3) Права доступа, только чтение
   * 4) Определяет способ отображения, изменение в памяти не буду затрагивать файл
